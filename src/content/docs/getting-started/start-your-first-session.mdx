...
When Plan mode returns a plan, Jean shows it in a dialog before any execution. The primary action is **YOLO**, with **Approve** available as the more conservative build-mode path.

The approval menu can also run the plan with a different backend or model for that one approval. Use this when you want a careful planning model to draft the plan, then hand execution to a faster build or yolo model.

Approval options include:
- **YOLO** to approve and continue in trusted yolo mode
- **Approve** to approve and continue in build mode
...
- **Other model…** to pick a one-shot backend/model override before starting execution

The clear-context and new-worktree options are useful when a plan grew out of exploration but deserves a clean execution environment.
