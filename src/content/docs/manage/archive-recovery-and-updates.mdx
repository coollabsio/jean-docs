...
- finished sessions you have not reopened appear in the unread popover
- `Cmd/Ctrl+Shift+F` opens the finished/unread sessions popover
- the popover can open a session, mark one item read, or mark everything read
...
- AppImage artifact support for Linux updates

Jean also tracks CLI versions separately from the app. When a supported backend CLI (Claude, Codex, OpenCode, Cursor, PI, Command Code, Grok, Kimi Code) has an update, Jean shows a CLI update modal so you can refresh the backend without leaving the app.

## Quit confirmation

If sessions are still executing when you quit, Jean shows a confirmation dialog listing the active work.

That prevents accidental cancellation of long-running agent runs.
