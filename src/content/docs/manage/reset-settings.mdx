...
## Clear backend credentials

Some backends store auth tokens outside Jean's config directory, so a config wipe alone may not stop a stale auth loop.

**macOS Keychain (Claude CLI):**

```bash
security delete-generic-password -s "Claude Code-credentials"
```

**Other backends:** log out through the backend's own CLI before relaunching Jean.

```bash
codex logout
# or, depending on the backend
opencode logout
```
