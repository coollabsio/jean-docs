...
If you bind Jean to `0.0.0.0` or another non-loopback host, treat it like any other developer tool exposed on your network. Keep token protection on. Prefer [Tailscale](https://tailscale.com/) (or WireGuard / SSH tunnel / reverse proxy with TLS) instead of a raw public port.
</Callout>

## What the browser mode does

Every Jean instance — desktop or `jean-server` — can serve the frontend over an embedded HTTP server and keep state in sync over WebSocket.
...
- connecting the native desktop app to a remote Jean instance (title bar server icon → **Add remote**)
