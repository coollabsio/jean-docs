...
When a supported backend is already working, Jean keeps follow-up input attached to the same session instead of losing it. Codex, OpenCode, and PI can steer plain text follow-ups into a running turn when auto-steer is enabled; other follow-ups are queued and sent when the current run finishes.

The chat also shows steered prompts as their own rows so you can tell what was added mid-run.
